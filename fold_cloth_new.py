...
Load rendered figures to a folder.
In order to run the code, need to install modules again in /PATH/TO/BLENDER/python/bin
Use ./python -m pip install MODULE
'''
...
def weight_group(mesh, pin_group):
    '''
    Add a vertex group weight editing modifier
    '''
...
def fold_cloth(dx, dy, dz, weight_mod, mid_frame, end_frame, start_frame = 5):
    '''
    ...
    The hook releases the cloth at the end (5 frames after placing).
    Return the scene object.
    Must be called after some vertex group is selected (will work on this problem issue). 
    '''
...
def fold(Dz, M_frame, E_frame):
    '''
    Create and fold the cloth based on picking height and folding speed
    '''
    cloth, cloth_physics = initialize()
    pin, unpin = select_single(cloth, cloth_physics, X, Y)
    weight_modifier = weight_group(cloth, pin)
    scn = fold_cloth(dx, dy, Dz, weight_modifier, mid_frame = M_frame, end_frame = E_frame)
    return cloth, scn

def read_z_value(mesh):
    # read vertex coordinate info:
    depgraph = bpy.context.evaluated_depsgraph_get()
    bm = bmesh.new()
    bm.verts.ensure_lookup_table()
    bm.from_object(mesh, depgraph) # read the evaluated (deformed) mesh data into the bmesh object:
    sum_of_z = 0
    highest_z = 0
...
def render_img(scn, k, Dz, E_frame):
    # change viewpoint to top-down view    
    ...
    # add camera
    bpy.ops.object.camera_add(enter_editmode=False, align='VIEW', location=(0, 0, 8), rotation=(0, -0, 0), scale=(1, 1, 1))
    scn.camera = bpy.context.object

    # render and output images as jpeg files
    bpy.context.scene.render.image_settings.file_format='JPEG'
    bpy.context.scene.render.filepath = "C:/Users/dongx/Desktop/folded_imgs/%d_dz_%.2f_frame_%d.jpg"%(k, Dz[k], E_frame[k]) # YOUR OWN PATH
    bpy.ops.render.render(write_still = True, use_viewport = True)
    img = np.array(image.imread(bpy.context.scene.render.filepath))
    return img

def iteration(dz_list, mid_frame_list, end_frame_list):
    sum_z_values = [] # a list containing the sum of z values
    folded_img = [] # a list of img
    # Iterations
    for k in range(N):
        cloth, scn = fold(Dz = dz_list[k], M_frame = mid_frame_list[k], E_frame = end_frame_list[k])
        sum_of_z_val = read_z_value(cloth)
        sum_z_values.append(sum_of_z_val)
        img = render_img(scn, k, dz_list, end_frame_list)
        folded_img.append(img)
    return sum_z_values, folded_img

sum_z_list, img_list = iteration(dz, MID_FRAME, END_FRAME)
'''
sum_z_list and img_list are for downstream tasks
'''
